...
def get_transaction(conn, transaction_id):
    return conn.run(
        conn.collection("transactions").find_one({"id": transaction_id}, {"_id": 0})
    )
...
def get_transactions(conn, transaction_ids):
    try:
        return conn.run(
            conn.collection("transactions").find(
                {"id": {"$in": transaction_ids}}, projection={"_id": False}
            )
        )
    except IndexError:
        pass
...
def get_asset(conn, asset_id):
    try:
        return conn.run(
            conn.collection("assets").find_one({"id": asset_id}, {"_id": 0, "id": 0})
        )
    except IndexError:
        pass
...
def get_assets(conn, asset_ids):
    return conn.run(
        conn.collection("assets").find(
            {"id": {"$in": asset_ids}}, projection={"_id": False}
        )
    )
...
def get_latest_block(conn):
    return conn.run(
        conn.collection("blocks").find_one(
...
@register_query(LocalMongoDBConnection)
def get_txids_by_operation(conn, operation):

    cursor = conn.run(
        conn.collection("transactions").find(
            {"operation": operation}, {"id": 1, "_id": False}
        )
    )
...
def get_block(conn, block_id):
    return conn.run(
        conn.collection("blocks").find_one(
            {"height": block_id}, projection={"_id": False}
        )
    )
...
def get_block_with_transaction(conn, txid):
    return conn.run(
        conn.collection("blocks").find(
            {"transactions": txid}, projection={"_id": False, "height": True}
        )
    )
...
def store_validator_set(conn, validators_update):
    ...
    return conn.run(
        conn.collection("validators").replace_one(
            {"height": height}, validators_update, upsert=True
        )
    )
...
def get_election(conn, election_id):
    ...
    return conn.run(
        conn.collection("elections").find_one(
            query, projection={"_id": False}, sort=[("height", DESCENDING)]
        )
    )
...
def get_latest_abci_chain(conn):
    return conn.run(
        conn.collection("abci_chains").find_one(
            projection={"_id": False}, sort=[("height", DESCENDING)]
        )
    )
